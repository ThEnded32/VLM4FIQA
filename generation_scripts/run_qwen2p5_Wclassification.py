...
try:
    ...
    print("       Without it, this script cannot build vision inputs for Qwen-VL.")
...
SYSTEM_PROMPT = (
    ...
    '"Lighting": "Balanced|Slightly Dark|Moderately Dark|Very Dark|Slightly Bright|Moderately Bright|Very Bright", '
    '"Compression": "None|Minimal|Moderate|Severe"}\n'
    "Rules: Output exactly the JSON object and nothing else."
)
...
def load_model_and_processor(model_name, device, use_flash_attn):
    ...
    # make generation greedy & warning-free
    model.generation_config.do_sample = False
    model.generation_config.temperature = 1.0
    model.generation_config.num_beams = 1
    model.generation_config.use_cache = True

    # use the fast processor explicitly
...
def run_on_folder(folder, model, processor, device, batch_size, max_new_tokens):
    ...
    if not images:
        return pd.DataFrame(columns=["image_path", "output_text", "quality_score"])
    ...
    for i in tqdm(range(0, len(images), batch_size)):
        ...
        try:
            ...
            generated_text = [processor.decode(ids, skip_special_tokens=True) for ids in generated_ids]

        for pth, out_text in zip(batch_paths, generated_text):
            ...
            if isinstance(parsed, dict):
                ...
                try:
                    ...
                    q = None
            rows.append({"image_path": str(pth), "output_text": out_text, "quality_score": q})
...
def main():
    ...
    ap.add_argument("--output_folder", type=str, default="classification_csv_outputs")
    ...
    folders = [d for d in find_image_dirs(datasets_root)]
    if not folders:
